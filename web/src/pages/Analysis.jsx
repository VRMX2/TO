      ...
      <div className="main-content" style={{ gridColumn: '1 / -1', padding: '1.5rem', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              <h2 className="text-primary font-mono" style={{ fontSize: '1rem', margin: 0, letterSpacing: '0.1em' }}>DEEP EQUILIBRIUM ANALYSIS</h2>
              <p className="text-secondary" style={{ fontSize: '0.72rem', margin: 0, marginTop: 2 }}>Zero-sum cyber-security game · Attacker vs Defender · Click matrix cells to edit</p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            ...
            <button onClick={reset} style={btnSt('rgba(255,255,255,0.05)', 'var(--text-muted)')}>RESET</button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            {/* Payoff Matrix */}
            <Panel color="cyan" title="PAYOFF MATRIX" subtitle="Click any cell to edit · NE = Nash Eq · PO = Pareto Optimal">
              <p className="text-muted font-mono" style={{ fontSize: '0.62rem', marginBottom: '0.75rem' }}>
                Row player = Attacker (maximizer) · Column player = Defender (minimizer) · Zero-sum: u_D = −u_A
              </p>
            ...
            <Panel color="amber" title="MIXED STRATEGY PROBABILITIES">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <div className="text-red font-mono" style={{ fontSize: '0.62rem', marginBottom: '0.5rem' }}>ATTACKER σ_A*</div>
                  {p.map((prob, i) => (
                ...
                <div>
                  <div className="text-cyan font-mono" style={{ fontSize: '0.62rem', marginBottom: '0.5rem' }}>DEFENDER σ_D*</div>
                  {q.map((prob, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: '0.4rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            {/* Pure Nash Table */}
            <Panel color="amber" title="PURE NASH EQUILIBRIA" badge={pureNash.length === 0 ? 'NONE FOUND' : `${pureNash.length} FOUND`}>
              <Collapse open={showInfo.pure} onToggle={() => toggle('pure')}>
                <strong>Definition:</strong> (s*_A, s*_D) is a Pure NE iff:<br />
                &nbsp;&nbsp;① u_A(s*_A, s*_D) ≥ u_A(s_A, s*_D) for all s_A &nbsp;<em>(attacker best-response)</em><br />
                &nbsp;&nbsp;② u_D(s*_A, s*_D) ≤ u_D(s*_A, s_D) for all s_D &nbsp;<em>(defender best-response)</em><br />
                In zero-sum games this equals a <strong>saddle point</strong>: column max AND row min simultaneously.<br />
                <strong>Minimax theorem:</strong> If a saddle point exists, v* = max_A min_D u = min_D max_A u.
              </Collapse>
              {pureNash.length === 0 ? (
                <div style={{ padding: '1rem', textAlign: 'center', border: '1px dashed rgba(255,214,10,0.25)', borderRadius: 6 }}>
                  <p className="text-amber font-mono" style={{ fontSize: '0.72rem', margin: 0 }}>NO PURE NASH EQUILIBRIUM EXISTS</p>
                  <p className="text-muted" style={{ fontSize: '0.62rem', marginTop: '0.5rem', lineHeight: 1.6 }}>
                    No saddle point found in this matrix.<br />The game is solved only in mixed strategies.
                  </p>
                </div>
              ) : (
            ...
            {/* Mixed Nash Table */}
            <Panel color="cyan" title="MIXED NASH EQUILIBRIUM" badge="UNIQUE (von Neumann)">
              <Collapse open={showInfo.mixed} onToggle={() => toggle('mixed')}>
                <strong>Definition:</strong> σ* = (σ*_A, σ*_D) is a Mixed NE iff:<br />
                &nbsp;&nbsp;E[u_A(σ*_A, σ*_D)] ≥ E[u_A(σ_A, σ*_D)] for all mixed σ_A<br />
                <strong>Indifference principle:</strong> At equilibrium, every pure strategy in the support yields equal expected payoff.<br />
                <strong>Minimax theorem (von Neumann 1928):</strong> max_σA min_σD E[u] = min_σD max_σA E[u] = v*<br />
                <em>Solved via iterative best-response (fictitious play). For exact results use LP.</em>
              </Collapse>
              <table style={TBLST}>
                <thead>
                  ...
                  {p.map((prob, i) => {
                    ...
                    return (
                      <tr key={`a${i}`} style={i % 2 === 0 ? { background: 'rgba(255,255,255,0.02)' } : {}}>
                        ...
                        <td style={TD}><Pill color={inSupport ? 'red' : 'muted'}>{inSupport ? 'IN SUPPORT' : 'EXCLUDED'}</Pill></td>
                  ...
                  {q.map((prob, i) => {
                    ...
                    return (
                      <tr key={`d${i}`} style={i % 2 === 0 ? { background: 'rgba(255,255,255,0.02)' } : {}}>
                        ...
                        <td style={TD}><Pill color={inSupport ? 'cyan' : 'muted'}>{inSupport ? 'IN SUPPORT' : 'EXCLUDED'}</Pill></td>
              ...
              <div style={{ marginTop: '0.6rem', padding: '0.5rem 0.75rem', background: 'rgba(0,240,255,0.05)', border: '1px solid rgba(0,240,255,0.15)', borderRadius: 6 }}>
                <span className="font-mono text-secondary" style={{ fontSize: '0.62rem' }}>
                  ...
                  &nbsp;— Attacker guarantees ≥ v*, Defender guarantees ≤ v*.
                </span>
              </div>
            </Panel>

            {/* Pareto Table */}
            <Panel color="green" title="PARETO OPTIMAL PROFILES" badge={`${pareto.length} PROFILES`}>
              <Collapse open={showInfo.pareto} onToggle={() => toggle('pareto')}>
                <strong>Definition:</strong> (s_A, s_D) is Pareto optimal iff ∄ (s'_A, s'_D) such that:<br />
                &nbsp;&nbsp;u_A(s') ≥ u_A(s) AND u_D(s') ≥ u_D(s) with at least one strict inequality.<br />
                In zero-sum games u_D = −u_A, so profiles maximizing u_A are Pareto optimal.<br />
                <strong>Note:</strong> Nash equilibria need not be Pareto optimal (Prisoner's Dilemma paradox), but in zero-sum games the Nash value is always Pareto efficient on the payoff frontier.
              </Collapse>
              <table style={TBLST}>
                <thead>
              ...
              <div style={{ marginTop: '0.6rem', padding: '0.5rem 0.75rem', background: 'rgba(0,255,102,0.04)', border: '1px solid rgba(0,255,102,0.15)', borderRadius: 6 }}>
                <span className="font-mono text-secondary" style={{ fontSize: '0.62rem' }}>
                  {pareto.length} Pareto-optimal profiles found. {pureNash.length > 0 ? `Pure NE at ${pureNash.map(n => `(A${n.row + 1},D${n.col + 1})`).join(', ')} lies on the Pareto frontier.` : 'No pure NE — mixed equilibrium lies in the Pareto interior.'}
                </span>
              </div>
            </Panel>
